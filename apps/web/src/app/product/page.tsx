  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center space-y-4">
        ...
        <p className="text-slate-600 font-medium text-sm">กำลังโหลดข้อมูลราคาสินค้าและประวัติราคา...</p>
      </div>
    );
  }
  ...
  if (!detail || !detail.product) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center space-y-6">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900">ไม่พบข้อมูลสินค้านี้</h1>
          <p className="text-slate-500 text-sm mt-1">สินค้าที่คุณค้นหาอาจไม่มีอยู่ในระบบ หรือ URL ไม่ถูกต้อง</p>
        </div>
        <Link
          href="/search"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm shadow-md transition-all"
        >
          <ArrowLeft className="w-4 h-4" /> กลับไปค้นหาสินค้า
        </Link>
      </div>
    );
  }
  ...
  return (
    <Suspense
      fallback={
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center space-y-4">
          <Loader2 className="w-10 h-10 text-orange-500 animate-spin mx-auto" />
          <p className="text-slate-500 text-sm">กำลังโหลดหน้ารายละเอียดสินค้า...</p>
        </div>
      }
    >
      <ProductContent />
